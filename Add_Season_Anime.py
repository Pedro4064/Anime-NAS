...
def update_database_season_anime():
    ...
    twist_moe = Moe()
    # Returns a list of dicts with anime_title and main_url
    raw_website_data = twist_moe.get_all_animes_in_database()

    # format the scraped data
    website_data = [{'anime_title': anime['anime_title']}
                    for anime in raw_website_data]
    ...
    new_anime_title = [
        title for title in website_titles if title not in database_titles]

    # Create a new list with all the info for the new anime
    new_anime = [{'anime_title': anime['anime_title'], 'main_url':anime['main_url']}
                 for anime in website_data if anime['anime_title'] in new_anime_title]
    ...
    for anime_title in new_anime_title:
        ...
        data_base, my_cursor = sql_connector()
        my_cursor.execute(
            "SELECT anime_id,anime_title FROM Animes WHERE anime_title='%s'" % (anime_title))
        raw_anime_info = my_cursor.fetchall()
        anime_info = convert_tuple(data=raw_anime_info, keys=[
                                   'anime_id', 'anime_title'], return_type='DICT')
    ...
    for anime in new_anime_info:
        ...
        try:
            ...
            anime_info = {
                'anime_id': anime['anime_id'], 'cover_path': cover_url}
