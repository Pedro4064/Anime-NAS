...
def get_watching_list():
    ...
    # Parse the data
    watching_list = convert_tuple(
        data=raw_data, keys=['anime_id'], return_type='LIST')
...
def get_main_url_from_id(anime_id: int):
    ...
    # get the raw data
    myCursor.execute(
        'SELECT main_url FROM Animes WHERE anime_id = %d' % (anime_id))
    raw_data = myCursor.fetchall()

    # format the data
    anime_url = convert_tuple(
        data=raw_data, keys=['main_url'], return_type='DICT')
...
def get_downloaded_episodes(anime_id: int):
    ...
    # get raw data
    myCursor.execute(
        'SELECT anime_id,episode_number FROM Downloads WHERE anime_id = %d' % (anime_id))
    raw_data = myCursor.fetchall()

    # format the data
    anime_episodes = convert_tuple(
        data=raw_data, keys=['anime_id', 'episode_number'], return_type='LIST')
...
def parse_new_episodes(downloaded_episodes: 'list of all episodes downloaded for that anime', released_episodes: 'list of all episodes released and available in the site'):
    ...
    if len(downloaded_episodes) == len(released_episodes):
        ...
        number_of_new_episodes = len(
            released_episodes)-len(downloaded_episodes)
...
def download_new_releases(anime_id, number_of_downloaded_episodes, new_releases_referers: list, new_releases_raw_mp4: list):
    ...
    for raw_url, referer in zip(new_releases_raw_mp4, new_releases_referers):
        ...
        # Update the database
        MyCursor.execute("INSERT INTO Downloads(anime_id,episode_number,file_path) VALUES(%d,%d,'%s')" % (
    ...
    while True:
        ...
        for anime_id in watching_list:
            ...
            try:
                ...
                raw_mp4_urls, referers = get_anime_links(
                    anime_url=main_anime_url)

                # Get only the data for the newly added episodes
                referers = parse_new_episodes(
                    downloaded_episodes=downloaded_episodes, released_episodes=referers)
                raw_mp4_urls = parse_new_episodes(
                    downloaded_episodes=downloaded_episodes, released_episodes=raw_mp4_urls)
                ...
                download_new_releases(anime_id=anime_id, number_of_downloaded_episodes=len(
                    downloaded_episodes), new_releases_referers=referers, new_releases_raw_mp4=raw_mp4_urls)
